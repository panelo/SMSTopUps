...
(function($) {
    ...
    $.Autocompleter = function($elem, options) {
		...
		$elem.keydown(function(e) {
            ...
            case 8: // backspace. (fix for autocomplete list not getting erased when all data is erased (or value becoming < minChars)). -- Added by Janak Porwal, 10-Nov-2010
            if (self.active_) {
               var value = self.dom.$elem.val();
               if (value.length <= self.options.minChars) {
                  value = value.slice(0, -1);
				      self.dom.$elem.val(value);
                  e.preventDefault();
                  self.finish();
                  return false;
               }
